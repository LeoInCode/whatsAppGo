...
func main() {
	...
	connStr := os.Getenv("POSTGRES_URL_CONNECTION")

	db, err := sql.Open("postgres", connStr)

	if err != nil {
		log.Fatal().Err(err).Msg("Could not open/create")
		os.Exit(1)
	}
	defer db.Close()

	sqlStmt := `CREATE TABLE IF NOT EXISTS users (id SERIAL NOT NULL PRIMARY KEY, name TEXT NOT NULL, token TEXT NOT NULL, webhook TEXT NOT NULL default '', jid TEXT NOT NULL default '', qrcode TEXT NOT NULL default '', connected INTEGER, expiration INTEGER, events TEXT NOT NULL default 'Message');`
	...
	if *waDebug != "" {
		...
		container, err = sqlstore.New("postgres", connStr, nil)
	}
	if err != nil {
		panic(err)
	}
